...
dropout_width = 45
dropout_height = 136.7
dropout_depth = 23.7
dropout_m_axle_pos = 42.5
...
def get_dropout_holes(ht):
	while(True):
		if ht == HoleType.tap:
			d = hole_tap_d[5]
			n = cylinder(r=d/2, h=nut_m5_len, center=True).rotateX(deg(90))
...
def get_dropout(is_threaded = True):
	...
	sole = sole.fillet(r=sole_thick/2, refs=[(0, -dropout_sole_size/2, sole_thick/2)])
	sole = sole.fillet(r=small_rounding_r, refs=[(0, dropout_sole_size/2, -sole_thick/2), (0, -dropout_sole_size/2, -sole_thick/2)])
	...
	res -= cylinder(r=wheel_axle_small_d/2, h=dropout_depth, center=True).rotateX(deg(90)).up((dropout_height)/2 - dropout_m_axle_pos)
	hblock = 4.5
	hd = hblock+4.9#7.8
	res -= cylinder(r=wheel_axle_big_d/2, h=dropout_depth-hd, center=True).rotateX(deg(90)).up((dropout_height)/2 - dropout_m_axle_pos).back(hd/2)
	a = 8.4
	b = 9.0
	c = 11.8
	...
	hdc = hblock+3
	cut = cylinder(r=wheel_axle_big_d/2, h=dropout_depth-hdc, center=True).rotateX(deg(90))
	...
	cut = cut.fillet(r = pw/2.01, refs=[(0, 1, ph/2), (0, -1, ph/2), (0, 1, -ph/2), (0, -1, -ph/2)])
	...
	cut = cut.fillet(r = pw/2.01, refs=[(0, ph/2, 1), (0, ph/2, -1), (0, -ph/2, 1), (0, -ph/2, -1)])
	#res -= cut.up(L-pw/2).back(-dropout_depth/2-pw+a+hblock + ph/2)
	
	res -= cylinder(r=gap(pedal_axis_d, 2)/2, h=dropout_width, center=True).rotateY(deg(90)).up(L).back(dropout_p_axle_pos - dropout_depth/2)
